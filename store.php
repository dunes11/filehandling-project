<?php
    ...
    if(isset($_POST["submit"])){
        if(($_FILES["myfile"]['error']!=4)){
           ...
           $to1="image/".date("Y-m-d").$_POST['username'].rand().$_FILES['myfile']["name"];
           $from=$_FILES['myfile']['tmp_name'];
           if(move_uploaded_file($from,$to1)){
     ...
     $sql="insert into customer(name,username,mobile,email,address,father_name,mother_name,dob,gender,profile)
        value('$_POST[name]','$_POST[username]','$_POST[mobile]','$_POST[email]','$_POST[address]','$_POST[father_name]','$_POST[mother_name]','$_POST[dob]','$_POST[gender]','$to1')";
    if($con->query($sql)){
        //echo "data inserted";
